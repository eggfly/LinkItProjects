...
void ctp_i2c_configure(VMUINT32 slave_addr, VMUINT32 speed) {
	...
	if (gpio_ctp_i2c_sda_pin == 0xFF) // HW I2C
		...
		vm_dcl_control(ctp_i2c_handle, VM_DCL_I2C_CMD_CONFIG,
				(void *) &conf_data);
	} else {
...
VM_DRV_TP_BOOL ctp_i2c_send_byte(VMUINT8 ucData) {
	...
	if (gpio_ctp_i2c_sda_pin == 0xFF) // HW I2C
		...
		if (ctp_i2c_configure_done) {
			...
			status = vm_dcl_control(ctp_i2c_handle, VM_DCL_I2C_CMD_CONT_WRITE,
					(void *) &write);
		}
		...
		for (i = 0, ucMask = 0x80; i < 8; i++, ucMask >>= 1) {
			if (ucData & ucMask)
				vm_dcl_control(sda_handle, VM_DCL_GPIO_COMMAND_WRITE_HIGH,
						NULL);
			else
				vm_dcl_control(sda_handle, VM_DCL_GPIO_COMMAND_WRITE_LOW, NULL);
			CTP_I2C_DELAY
		...
		vm_dcl_control(sda_handle, VM_DCL_GPIO_COMMAND_READ,
				(void *) &sda_read);
		...
		ret = (sda_read.level_status == VM_DCL_GPIO_IO_LOW) ?
				(VM_DRV_TP_BOOL) CTP_I2C_ACK : (VM_DRV_TP_BOOL) CTP_I2C_NAK;
	}
	return ret;
}
...
VMUINT8 ctp_i2c_receive_byte(VM_DRV_TP_BOOL bAck) {
	...
	if (gpio_ctp_i2c_sda_pin == 0xFF) // HW I2C
		...
		if (ctp_i2c_configure_done) {
			...
			status = vm_dcl_control(ctp_i2c_handle, VM_DCL_I2C_CMD_CONT_READ,
					(void *) &read);
...
VM_DRV_TP_BOOL ctp_i2c_send(VMUINT8 ucDeviceAddr, VMUINT8 ucBufferIndex,
...
VM_DRV_TP_BOOL ctp_i2c_send_ext(VMUINT8 ucDeviceAddr, VMUINT16 ucBufferIndex,
		VMUINT8* pucData, VMUINT32 unDataLength) {
	VMUINT32 i;
	...
	if (gpio_ctp_i2c_sda_pin == 0xFF) // HW I2C
		...
		if (ctp_i2c_configure_done) {
			while (offset <= unDataLength) {
				...
				write.transfer_number = 1;
				status = vm_dcl_control(ctp_i2c_handle,
						VM_DCL_I2C_CMD_CONT_WRITE, (void *) &write);
				if (status != VM_DCL_STATUS_OK)
					return VM_DRV_TP_FALSE;
...
VM_DRV_TP_BOOL ctp_i2c_receive(VMUINT8 ucDeviceAddr, VMUINT8 ucBufferIndex,
		VMUINT8* pucData, VMUINT32 unDataLength) {
	VMUINT32 i;
	...
	if (gpio_ctp_i2c_sda_pin == 0xFF) // HW I2C
		...
		if (ctp_i2c_configure_done) {
			...
			dcl_i2c_ret = vm_dcl_control(ctp_i2c_handle,
					VM_DCL_I2C_CMD_WRITE_AND_READ, (void *) &write_read);
			...
			if (ctp_i2c_send_byte(ucBufferIndex) == CTP_I2C_ACK) {
				...
				if (ctp_i2c_send_byte(ucDeviceAddr | 0x01) == CTP_I2C_ACK) {
					for (i = 0; i < unDataLength - 1; i++) {
						pucData[i] = ctp_i2c_receive_byte(
								(VM_DRV_TP_BOOL) CTP_I2C_ACK);
					}
					pucData[unDataLength - 1] = ctp_i2c_receive_byte(
							(VM_DRV_TP_BOOL) CTP_I2C_NAK);
				} else {
					bRet = VM_DRV_TP_FALSE;
				}
			} else {
...
VM_DRV_TP_BOOL ctp_i2c_receive_ext(VMUINT8 ucDeviceAddr, VMUINT16 ucBufferIndex,
		VMUINT8* pucData, VMUINT32 unDataLength) {
	VMUINT32 i;
	VM_DRV_TP_BOOL bRet = VM_DRV_TP_TRUE;
	vm_dcl_i2c_control_write_and_read_t write_read;
	...
	if (gpio_ctp_i2c_sda_pin == 0xFF) // HW I2C
		...
		if (ctp_i2c_configure_done) {
			while (offset < unDataLength) {
				...
				vm_dcl_control(ctp_i2c_handle, VM_DCL_I2C_CMD_WRITE_AND_READ,
						(void *) &write_read);
				offset += pkt_len;
